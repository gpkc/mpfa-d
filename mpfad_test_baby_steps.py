...
def _boundary_cross_term(tan_vector, norm_vector, face_area,
...
face_verts = [[verts[0], verts[1], verts[2]],
              [verts[0], verts[1], verts[3]],
              [verts[0], verts[2], verts[3]],
              [verts[1], verts[2], verts[3]]]
perm = np.asarray(benchmark_1(0., 0., 0.)[0])
...
for face in face_verts:
    ...
    D_JI = _boundary_cross_term(tan_JK, JR, face_area,
                                K_R_JK, K_R_n, h_R)
    D_JK = _boundary_cross_term(tan_JI, JR, face_area,
                                K_R_JI, K_R_n, h_R)

    K_n_eff = K_R_n / h_R
    flux.append(-(2 * K_n_eff * (u_c - g_J) + D_JI * (g_J - g_I) + D_JK * (g_J - g_K)))
