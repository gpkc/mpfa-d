...
def get_mocked_method(mocker):
    mocker.patch(
        "mpfad.interpolation.InterpolationMethod.InterpolationMethodBase._set_nodes"
    )
    mocker.patch(
        "mpfad.interpolation.InterpolationMethod.InterpolationMethodBase._set_faces"
    )
...
class TestIDW:
    def test_calc_weight(self, idw_method):
        ...
        vol_centroid = np.array([4.0, 0.0, 0.0])
...
class TestLSW:
    def test_calc_G(self, lsw_method):
        ...
        G = lsw_method.calc_G(59.0, -13.0, 9, 11.0, 1.0, 3.0,)

        assert G == pytest.approx(256.0)

    def test_calc_psi(self, lsw_method):
        node_coords = np.array([0.0, 0.0, 0.0])
        vol_centroid = np.array([1.0, 0.0, 0.0])

        lsw_method.mesh_data.get_centroid.return_value = vol_centroid
        lsw_method.mesh_data.get_coords.return_value = node_coords
        R_x, R_y, R_z = 1.5, 1.58333333333333, 4.0
        I_xx, I_yy, I_zz = 1.125, 0.923611111111111, 6
        I_xy, I_xz, I_yz = 0.958333333333333, 1.75, 1.91666666666667
        G = 0.19140625

        psi_x = lsw_method.calc_psi(
            R_x, R_y, R_z, I_xy, I_xz, I_yy, I_yz, I_zz, G
        )
        psi_y = lsw_method.calc_psi(
            R_y, R_x, R_z, I_xy, I_yz, I_xx, I_xz, I_zz, G
        )
        psi_z = lsw_method.calc_psi(
            R_z, R_x, R_y, I_xz, I_yz, I_xx, I_xy, I_yy, G
        )
    ...
    def test_interpolate(self, lsw_method):
        ...
        assert weight[0] == pytest.approx(1 / 3.0)
        assert weight[1] == pytest.approx(4 / 9.0)
        assert weight[2] == pytest.approx(1 / 5.0)
