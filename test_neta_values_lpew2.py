...
nd = -1.
node_coords = np.asarray([nd, nd, nd])
...
for j in range(1, 7, 1):
    ...
    for r in range(1, 7, 1):
        ...
        n_k_hat = np.dot(N_face, np.dot(perm, N_face)) / (3. * volume)
        ...
        neta_k_hat = n_k_hat / (n_k_adj_plus - n_k_plus)
        neta = (n_k_adj - n_k) / (n_k_adj_plus - n_k_plus)
        phi = phi * neta

        prod = 1.
        for r_star in r_range(j, r):
            volume = _get_volume(node_coords, faces[r_star], tetra_c_coords)
            N_face = np.cross(T[r_star + 1], T[r_star]) / 2.

            N_i = np.cross(tetra_c_coords, T[r_star + 1]) / 2
            n_k = np.dot(N_i, np.dot(perm, N_face)) / (3. * volume)

            N_i = np.cross(T[r_star], tetra_c_coords) / 2
            n_k_plus = np.dot(N_i, np.dot(perm, N_face)) / (3. * volume)

            volume = _get_volume(node_coords, faces[r_star],
                                 adj_centroid[r_star])
            N_face_adj = N_face

            N_i_adj = np.cross(T[r_star + 1], adj_centroid[r_star]) / 2
            n_k_adj = np.dot(N_i_adj, np.dot(adj_perm,
                                             N_face_adj)) / (3. * volume)

            N_i_adj = np.cross(adj_centroid[r_star], T[r_star]) / 2
            n_k_adj_plus = np.dot(N_i_adj, np.dot(adj_perm,
                                                  N_face_adj)) / (3. * volume)
            neta_ratio = (n_k_adj - n_k) / (n_k_adj_plus - n_k_plus)
            prod = (-1) ** (1 + r_star) * neta_ratio * prod
        summ += neta_k_hat * prod
    print(summ)
            #print(j, r, n_k_adj, n_k, n_k_adj_plus, n_k_plus, neta_ratio)
